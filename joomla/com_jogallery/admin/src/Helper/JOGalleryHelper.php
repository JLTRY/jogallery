...
class JOGalleryImage
{
    ...
    public static function createfrom($object, $dirname) {
        ...
        return new JOGalleryImage($dirname, $object->filename, $object->basename, $object->moddate, $object->comment, $object->urlfilename, $object->urlshortfilename, $relative);
    }
    ...
    {
        $thumbimage = JThumbsHelper::getthumb(JOGalleryHelper::join_paths(JPATH_SITE, $this->dirname), $mode, $this->basename);
...
class JOGalleryHelper
{
    public static function loadLibrary($libraries = array('jquery' => true))
    {
        ...
        {
            $wa->useScript('com_jogallery.jimages');
            $wa->useStyle('com_jogallery.jimages');
        }
        if (isset($libraries['psw_images']))
        {
            $wa->useScript('com_jogallery.psw_images');
            $wa->useStyle('com_jogallery.psw_images');
        }
        if (isset($libraries['jogallery']))
        {
        ...
        {
            $wa->useScript('com_jogallery.jdirectories');
        }
        if (isset($libraries['jthumbs']))
        {
            $wa->useScript('com_jogallery.jthumbs');
        }
        if (isset($libraries['jrecthumbs']))
        {
            $wa->useScript('com_jogallery.jrecthumbs');
        }
        if (isset($libraries['jcomments']))
        {
            $wa->useScript('com_jogallery.jcomments');
        }
    ...
    public static function addSubmenu($submenu) 
    {
        ...
        $document = Factory::getDocument();
        $document->addStyleDeclaration('.icon-48-jogallery ' .
                                       '{background-image: url(../media/com_jogallery/images/tux-48x48.png);}');
    ...
    public static function join_paths(...$spaths)
    {
        ...
        //Log::add('join_paths=>:'. $prefix . implode(DIRECTORY_SEPARATOR, array_reverse($arjoinpath)), Log::WARNING, 'jogallery');
    ...
    {
        $jogallerydir = JOGalleryHelper::join_paths(JPATH_SITE, $rootdir,  $directory);
        $jogalleryfile = JOGalleryHelper::join_paths($jogallerydir, "gallery.csv");
        $exist = $modified = false;
        if (file_exists($jogalleryfile)) {
            $listfiles = JOGalleryImage::readcsv($jogalleryfile);
            $exist = true;
        }
        else {
            $listfiles = array();
        }
        foreach (JOGalleryImage::$IMG_EXTENSIONS as $ext) {
            foreach (glob($jogallerydir . "/*.$ext") as $filename) {
                ...
                array_push($listfiles, new JOGalleryImage(JOGalleryHelper::join_paths($rootdir, $directory),
                                            $pathinfo['filename'],
                                            basename($filename),
                                            $moddate,
                                            "",
        ...
        foreach (JOGalleryImage::$VIDEO_EXTENSIONS as $ext) {
            foreach (glob($jogallerydir . "/*.$ext") as $filename) {
                $pathinfo = pathinfo($filename);
                $moddate = filemtime($filename);
                $exist = false;
                ...
                if (!file_exists(JOGalleryHelper::join_paths($jogallerydir, $urlshortfilename))) {
        ...
        if (!$exist || $modified)
        {
            ...
            foreach ($listfiles as $file) {
                array_push($listfilteredfiles, JOGalleryImage::toArray($file, false));
            }
            usort($listfilteredfiles, array($cur_class,'sortFile'));
            JOGalleryImage::savecsv($jogalleryfile, $listfilteredfiles);
        }
        $listfilteredfiles = array();
        foreach ($listfiles as $file) {
            ...
            if (($startdate == -1) || (($moddate - $startdate) >= 0)){
                if (($enddate == -1 ) ||  (($moddate != -1) && ($enddate - $moddate) >= 0)) {
                    ...
                    if (($media == "ALL") || ($file->video && ($media == "VIDEOS")) || (!$file->video && ($media == "IMAGES"))) {
                        array_push($listfilteredfiles, JOGalleryImage::toArray($file, true));
                    }
                }
            }
        }
    ...
    public static function outputphotoswipe($id, $directory, $listfiles, $page, &$content)
    {
        ...
        JOGalleryHelper::loadLibrary(array("inline" => array('import {init_psw,psw_images_getimages} from "' . Uri::root() . '/media/com_jogallery/js/psw_images.js";
                                                        (function($) {
                                                                $(document).ready(function() {
    ...
    public static function outputimg($rootdir, $directory, $file, $name, $icon,$width, &$content)
    {
        ...
        if ($icon == 'small') {
            ...
            $urlshortfilename = JThumbsHelper::getthumburl( JOGalleryHelper::join_paths($rootdir, $directory), $icon, $file->basename);
        }
        if ($width == "?") {
...
// $dir is the full path sdir is the directory as parameter
    public static function outputdirs($galid, $id, $dir, $directory, $parentlevel, &$content, $type='radio', $media = "ALL", $lightbox = "fancybox") {
        ...
        $ret = $jroot->findDirs($dir, $directory, JODirectory::$_excludes, true);
    ...
    */	   
    public static function display( $_params)
    {
        ...
        if ( array_key_exists('img', $_params)) {
            ...
            //sub directories
            $sdir = html_entity_decode(JOGalleryHelper::join_paths(JPATH_SITE, $rootdir,  $directory));
    ...
    static function savecomments($directory, $jcomments) {
        ...
        $jogalleryfiles = JOGalleryHelper::getFiles($rootdir , $directory);
        $modif = false;
        foreach($jcomments as $array_key => $comment) {
            $i = 0;
            foreach($jogalleryfiles as $file)
            {
                if (($file['filename'] == $array_key)) {
                if ($file['comment'] != $comment) {
                        $modif = true;
                        $jogalleryfiles[$i]['comment'] = $comment;
                    }
                    break;
                }
                $i++;
        ...
        if ($modif)
        {
            $jogallerydir = JOGalleryHelper::join_paths(JPATH_SITE, $rootdir,  $directory);
            $jogalleryfile = JOGalleryHelper::join_paths($jogallerydir, "gallery.csv");
            JOGalleryImage::savecsv($jogalleryfile, $jogalleryfiles);
            return "savecsv ok";
        }
        else 
        {
    ...
    static function deleteimage($rootdirectory, $directory, $Image, $keep, &$errors)
    {
        ...
        Log::add("deleteimage:" . $filename, Log::WARNING, 'com_jogallery');
        if (file_exists($filename)){
            if ($keep) {
                array_push($errors, "keep " . $filename);
                Log::add("deleteimage:keep:" . $filename, Log::WARNING, 'com_jogallery');
            }
            else {
                unlink($filename);
                Log::add("deleteimage:delete:" . $filename, Log::WARNING, 'com_jogallery');
                array_push($errors, "success deleting " . $filename);
            }
        }
        else {
            array_push($errors, "file does not exist " . $filename);
            Log::add("deleteimage:dose not exist:" . $filename, Log::WARNING, 'com_jogallery');
        }
    }
    ...
    static function deleteimages($directory, $Images, $keep, &$errors)
    {
        ...
        {
            Log::add("delete" . $rootdir . "/" . $directory . "/" . $Image, Log::WARNING, 'com_jogallery'); 
            self::deleteimage($rootdir, $directory, $Image, $keep, $errors);
            Log::add("delete end:" . $rootdir . "/" . $directory . "/" . $Image, Log::WARNING, 'com_jogallery'); 
            JThumbsHelper::deletethumbs($rootdir, $directory, $Image, $errors);
            Log::add("delete end thumbs:" . $rootdir . "/" . $directory . "/" . $Image, Log::WARNING, 'com_jogallery'); 
            $i = 0;
            $found = false;
            foreach($jogalleryfiles as $file)
            {
            ...
            if ($found) {
                array_splice($jogalleryfiles, $i, 1);
                $modif = true;
            }
        }
        if ($modif) {
