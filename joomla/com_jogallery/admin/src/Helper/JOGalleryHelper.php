...
class JOGalleryHelper
{
    ...
    {
        return self::joinPaths(JPATH_SITE, JParametersHelper::getrootdir());
    }

    public static function guessDate($filename, &$date)
    {
        if (
            preg_match('/IMG[-_](\d{4})(\d{2})(\d{2})[-_](\d{2})(\d{2})(\d{2}).*/', $filename, $re) ||
            ...
            preg_match('/IMG[-_](\d{4})(\d{2})(\d{2}).*/', $filename, $re) ||
    ...
    {
        $jogallerydir = JOGalleryHelper::joinPaths(JPATH_SITE, $rootdir, $directory);
        $jogalleryfile = JOGalleryHelper::joinPaths($jogallerydir, "gallery.csv");
        ...
        foreach (JOGalleryImage::$IMG_EXTENSIONS as $ext) {
            foreach (glob($jogallerydir . "/*.$ext") as $filename) {
                ...
                array_push($listfiles, new JOGalleryImage(
        ...
        foreach (JOGalleryImage::$VIDEO_EXTENSIONS as $ext) {
            foreach (glob($jogallerydir . "/*.$ext") as $filename) {
                ...
                if (!file_exists(JOGalleryHelper::joinPaths($jogallerydir, $urlshortfilename))) {
        ...
        foreach ($listfiles as $file) {
            ...
            if (($startdate == -1) || (($moddate - $startdate) >= 0)) {
                if (($enddate == -1 ) ||  (($moddate != -1) && ($enddate - $moddate) >= 0)) {
                    //filter files
                    if (
                        ($media == "ALL") || ($file->video && ($media == "VIDEOS")) ||
                        (!$file->video && ($media == "IMAGES"))
                    ) {
...
// $dir is the full path sdir is the directory as parameter
    ...
    */
    ...
    public static function savecomments($directory, $jcomments)
    {
    ...
    public static function deleteimage($rootdirectory, $directory, $Image, $keep, &$errors)
    {
        $filename = self::joinPaths(JPATH_ROOT, $rootdirectory, $directory, $Image);
    ...
    public static function deleteimages($directory, $Images, $keep, &$errors)
    {
        $rootdir = JParametersHelper::getrootdir();
        $jogalleryfiles = JOGalleryHelper::getFiles($rootdir, $directory);
        $modif = false;
